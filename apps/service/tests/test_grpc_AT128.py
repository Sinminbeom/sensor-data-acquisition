...
from service_pb2 import Acquisition, Health, Connection, Sensor


class TestGrpcAT128(unittest.TestCase):
    ACQ_DURATION = 10
    loop = False
    ...
    def test_check_connection(self):
        connection = self.client.is_connected(Sensor(name='eth_e_a'))
        self.assertEqual(connection.state, Connection.State.CONNECTED)

    def test_acquisition(self):
        # Test start_acquisition
        acquisition = self.client.start_acquisition(Sensor(name='eth_e_a'))
        self.assertEqual(acquisition.name, 'eth_e_a')
        ...
        acquisition = self.client.is_acquiring(Sensor(name='eth_e_a'))
        self.assertEqual(acquisition.name, 'eth_e_a')
        self.assertEqual(acquisition.state, Acquisition.State.ACQUIRING)
        self.assertEqual(acquisition.reason, '')
    ...
    def __test_stop_acquisition(self):
        ...
        acquisition = self.client.stop_acquisition(Sensor(name='eth_e_a'))
        self.assertEqual(acquisition.name, 'eth_e_a')
        self.assertEqual(acquisition.state, Acquisition.State.NOT_ACQUIRING)
        self.assertEqual(acquisition.reason, '')
        self.loop = False
    ...
    def test_check_health(self):
        health = self.client.is_healthy(Sensor(name='eth_e_a'))
        self.assertEqual(health.name, 'eth_e_a')
